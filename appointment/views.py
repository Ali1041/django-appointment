...
from appointment.utils.db_helpers import (
    can_appointment_be_rescheduled,
    ...
    is_working_day,
    staff_change_allowed_on_reschedule,
...
def home(request):
    """This view function handles requests to the home page.

    :param request: The request instance.
    :return: The rendered HTML page.
    """
    return HttpResponse("Hello, world. You're at the appointment home page.")
...
def appointment_request(request, service_id=None, staff_member_id=None):
    ...
    page_description = _("Book an appointment for {s} at {wn}.").format(
        s=service.name, wn=get_website_name()
    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if not staff_exists:
                ...
                return redirect(
                    "appointment:appointment_client_information",
...
def redirect_to_payment_or_thank_you_page(appointment):
    ...
    """
    if (
        APPOINTMENT_PAYMENT_URL is not None and APPOINTMENT_PAYMENT_URL != ""
    ) and appointment.service_is_paid():
...
def create_appointment(request, appointment_request_obj, client_data, appointment_data):
    ...
    appointment = create_and_save_appointment(
        appointment_request_obj, client_data, appointment_data, request
    )
...
def appointment_client_information(request, appointment_request_id, id_request):
    ...
    has_required_email_reminder_config = (
        has_required_email_settings() and check_q_cluster(hide_warning=True)
    )
    ...
    if request.method == "POST":
        ...
        if appointment_form.is_valid() and client_data_form.is_valid():
            ...
            is_email_in_db = CLIENT_MODEL.objects.filter(
                email__exact=client_data["email"]
            ).exists()
            if is_email_in_db:
                return handle_existing_email(
                    request,
                    client_data,
                    appointment_data,
...
def enter_verification_code(request, appointment_request_id, id_request):
    ...
    if request.method == "POST":
        ...
        if verify_user_and_login(request, user, code):
            appointment_request_object = AppointmentRequest.objects.get(
                pk=appointment_request_id
            )
            appointment_data = get_appointment_data_from_session(request)
            response = create_appointment(
                request=request,
                appointment_request_obj=appointment_request_object,
                client_data={"email": email},
                appointment_data=appointment_data,
            )
            appointment = Appointment.objects.get(
                appointment_request=appointment_request_object
            )
            appointment_details = {
        ...
        if token_verification is not None:
            if request.method == "POST":
                ...
                if form.is_valid():
                    ...
                    context = get_generic_context_with_extra(
                        request, extra, admin=False
                    )
                    return render(
                        request, "appointment/thank_you.html", context=context
                    )
            else:
...
def prepare_reschedule_appointment(request, id_request):
    ...
    if not can_appointment_be_rescheduled(ar):
        url = reverse(
            "appointment:appointment_request", kwargs={"service_id": ar.service.id}
        )
        context = get_generic_context_with_extra(
            request,
            {
        ...
        return render(
            request,
            "error_pages/403_forbidden_rescheduling.html",
            context=context,
            status=403,
        )
    ...
    staff_filter_criteria = (
        {"id": ar.staff_member.id}
        if not staff_change_allowed_on_reschedule()
        else {"services_offered": ar.service}
    )
    ...
    page_description = _("Reschedule your appointment for {s} at {wn}.").format(
        s=service.name, wn=get_website_name()
    )
...
def reschedule_appointment_submit(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            client_first_name = Appointment.objects.get(
                appointment_request=ar
            ).client.first_name
            email = Appointment.objects.get(appointment_request=ar).client.email
            send_reschedule_confirmation_email(
                request=request,
                reschedule_history=arh,
                first_name=client_first_name,
                email=email,
                appointment_request=ar,
            )
            return render(
                request, "appointment/rescheduling_thank_you.html", context=context
            )
        else:
            messages.error(
                request,
...
def confirm_reschedule(request, id_request):
    reschedule_history = get_object_or_404(
        AppointmentRescheduleHistory, id_request=id_request
    )

    if (
        reschedule_history.reschedule_status != "pending"
        or not reschedule_history.still_valid()
    ):
        error_message = (
            _("O-o-oh! This link is no longer valid.")
            if not reschedule_history.still_valid()
            else _("O-o-oh! Can't find the pending reschedule request.")
        )
        context = get_generic_context_with_extra(
            request, {"error_message": error_message}, admin=False
        )
        return render(
            request, "error_pages/404_not_found.html", status=404, context=context
        )
